...
- A literal-IP host is checked directly; a named host is resolved (`dns.lookup`, all addresses) and rejected if **any** resolved address is private/reserved. The classification comes from [`ipaddr.js`](https://github.com/whitequark/ipaddr.js) (`addr.range()`, the SDK's only runtime dependency — 0 transitive deps, +13.4 KB minified per workflow bundle), and the rule over it is an **allow-list of one**: the address is refused unless the range is `unicast`. So loopback, `0/8`, the RFC 1918 private ranges, link-local (incl. the metadata address `169.254.169.254`), CGNAT `100.64/10`, multicast, broadcast, the documentation/benchmarking/future-use bands, IPv6 ULA `fc00::/7`, `fe80::/10`, deprecated site-local, and the transitional ranges 6to4 `2002::/16`, Teredo `2001::/32` and NAT64 `64:ff9b::/96` are all refused — including the ones nobody here enumerated. IPv4-mapped (`::ffff:a.b.c.d`) and deprecated IPv4-compatible (`::a.b.c.d`) forms are the one exception the rule needs: they are never `unicast` themselves, so `isIPv4MappedAddress()`/`toIPv4Address()` unwraps them and the embedded address is judged in its place. An address `ipaddr.parse` throws on is blocked (fail-closed). PO-183 replaced the hand-rolled IPv4/IPv6 parsers and range checks this used to carry; the residual is that `ipaddr.js` is pinned, so a range the registry sets aside after that version reads as `unicast` until the pin moves.
