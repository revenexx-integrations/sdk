...
The SDK has one runtime dependency (`ipaddr.js`, for the SSRF guard's address classification) and no logic beyond the manifest helpers. It exists solely to share the contract between node packages and the workflow engine.
...
- A literal-IP host is checked directly; a named host is resolved (`dns.lookup`, all addresses) and rejected if **any** resolved address is private/reserved. The classification comes from [`ipaddr.js`](https://github.com/whitequark/ipaddr.js) (`addr.range()`, the SDK's only runtime dependency — 0 transitive deps, +13.4 KB minified per workflow bundle, pinned to an exact version), and the rule over it is an **allow-list**: the address is refused unless it is public unicast. So loopback, `0/8`, the RFC 1918 private ranges, link-local (incl. the metadata address `169.254.169.254`), CGNAT `100.64/10`, multicast, broadcast, the documentation/benchmarking/future-use bands, IPv6 ULA `fc00::/7`, `fe80::/10`, deprecated site-local, and the transitional ranges 6to4 `2002::/16` and Teredo `2001::/32` are all refused — including the ones nobody here enumerated.
  - **`range() === 'unicast'` is not on its own the rule, because that verdict is a *default*.** `subnetMatch` falls back to `'unicast'` for an address no special range matched, which for IPv4 is the same as being publicly routable (the space is fully allocated) but for IPv6 is not: `1000::/4`, `4000::/2`, `8000::/1` and `fe00::/9` are unallocated and answer `'unicast'` too. So the IPv6 half additionally requires `addr.match(ipaddr.parseCIDR('2000::/3'))` — IANA's global unicast allocation. That constant is the one piece of classification still kept here, and it fails safe: space allocated outside it later reads as refused, which costs reachability rather than safety.
  - **The forms that embed one version in the other are judged on the address they carry, not on the wrapper** — none of them is `unicast`, and none sits in `2000::/3`, so without unwrapping the rule would refuse them all on the notation. IPv4-mapped `::ffff:a.b.c.d`, the deprecated IPv4-compatible `::a.b.c.d` (which the parser normalises to the mapped form, so `range()` reports `ipv4Mapped` for both), and the NAT64 well-known prefix `64:ff9b::/96` (`rfc6052`) are unwrapped via the low 32 bits — `ipaddr.fromByteArray(addr.toByteArray().slice(12))`, not `toIPv4Address()`, which is defined only for the mapped form and throws on the NAT64 prefix. NAT64 is unwrapped rather than refused because on an IPv6-only network with DNS64 (the standard AWS/GCP IPv6-only setup) it is what *every* IPv4-only host resolves to; refusing the prefix whole would make every IPv4-only API unreachable. A private address behind any of these forms is still refused. 6to4 and Teredo stay refused whole — legacy transition, not infrastructure anything here runs on.
  - An address `ipaddr.parse` throws on is blocked (fail-closed). PO-183 replaced the hand-rolled IPv4/IPv6 parsers and range checks this used to carry; the residuals are the pin's vintage (a range the registry sets aside inside the allocated block reads as `unicast` until the pin moves) and the allocation boundary above. Three assumptions this leans on — the `'unicast'` range name, the mapped/compatible conflation, and the `rfc6052` name — are asserted directly in `src/ssrf.test.ts`, so a dependency bump that changes one goes red as that assumption rather than as a matrix of addresses quietly changing sides.
- The address the connection actually lands on is judged too. `safeFetch` subscribes to undici's `undici:client:connected` diagnostics channel while a hop is in flight and, for a **host and port** it is currently reaching, runs `isBlockedAddress` over `socket.remoteAddress`; a private peer gets `socket.destroy(…)`. The channel is published **synchronously, before the request is written**, so a refused target receives no request bytes at all — only the completed handshake (see the box below). Host **and** port, because that is as narrow as the scope goes: the message names the target, never the caller, so the guard can say which connections are candidates and not which one is its own.
...
>
> Two things keep that canary honest. It runs on **every Node major `engines` claims** (the `engines` matrix job in `ci.yml`, beside the `test` job that pins the newest), because the message shape is Node's to change and a canary that only ever ran on one major is not watching the others. And the guard **says so when the shape has changed under it**: a `connected` message whose `hostname`, `port` or `protocol` it cannot read is logged once, as `the connect-time half of the SSRF guard is not judging connections`. It is let through rather than refused — a message with no target cannot be told apart from the host process's own traffic, and refusing both is the process-wide policy this must not become — so this warning is the only signal that the half went quiet in production.
>
> **What remains: the handshake, and a connection somebody else opened.** A connection is judged once it stands, so the TCP handshake — and for `https:` the TLS handshake — with the refused address has already happened. Nothing of the request follows it, but whoever aimed the call there learns that something answers on that address and port, and a service that reacts to a bare connection has reacted. Second, connections are pooled per origin and only a *new* one is announced, so a socket something else in the process opened to the same host first could be reused unjudged — and the mirror image of that: while a call to a target is in flight, a connection *somebody else* opens to the same `host:port` is judged by the same rule and loses its socket if it lands somewhere private. Third, the registration lasts as long as the call and not as long as the connect, so a connect that lands after its fetch timed out is not judged either. All of them are recorded as gaps in the spec. A **network-level egress policy** on the worker is still the layer that does not depend on this code being right, and is worth having regardless.
>
...
>
> **Assumes direct egress.** The guard checks the *hostname's* resolved IPs. If a global proxy is ever installed (`undici.setGlobalDispatcher(new ProxyAgent(…))` / a `Dispatcher` that tunnels through a forward proxy), the real connection is opened by the proxy and the target is resolved *there* — the IP check no longer reflects where the bytes go, and neither does the connected-socket check, whose peer would be the proxy. It can also fail the opposite way — into an outage rather than a hole — because the connected-socket check refuses a peer address it cannot read: a `connect` over a **Unix socket** has no `remoteAddress` at all (verified), so every `safeFetch` call through such a dispatcher would throw `BLOCKED_ADDRESS`, and a forward proxy on a `10.x` address would present *its own* address as the peer and be refused as private. Which of the two a given `ProxyAgent` produces is **not verified** — this package has no `undici` dependency to probe with, and Node's built-in `fetch` cannot be repointed without one. Node's `fetch` does **not** honour `HTTP(S)_PROXY` by default, so none of this is reachable today; treat installing a global proxy dispatcher on the worker as a change that needs its own egress control **and** a test that `safeFetch` still works at all.
