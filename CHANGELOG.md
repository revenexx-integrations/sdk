...
- 2f00155: PO-537: a node package decides the order of its nodes inside a palette folder.
...
- 17fb19a: PO-516: the release toolchain moves to Changesets CLI v3, and the action that drives it to v2.
...
- db5ede8: PO-497: `maxBytesConfigField` says what the size cap is counted over.

  The description 1.4.0 shipped — _"The limit this package enforces applies above
  whatever is set here"_ — read worse in English than its German did, and neither
...
- a91b03f: PO-497: two helpers stop handing back something the caller has to correct.
  ...
  **`ApiKeyCredential.credentialShape()` now defaults to the name
  ...
  `apiKeyField()` alone used to change only which form field was _read_; `resolve()`
  ...
  _resolved_ blob and never calls `resolve()` — which is why it is fixed rather
  than documented.
  ...
  **This is a behaviour change, and the one case it alters is the broken one.** It
  ...
  The doc comment on `apiKeyField()` now says what it does _not_ do, and each
